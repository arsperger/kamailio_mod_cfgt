...
int _cfgt_get_post_url(str uuid, str *dest, int *end)
{
	char *format = "%.*s%.*s";
	if(dest == NULL || uuid.len == 0)
		return -1;

	dest->len = cfgt_sipmock_url.len + uuid.len;
	if (cfgt_sipmock_url.s[cfgt_sipmock_url.len - 1] != '/') {
		dest->len = dest->len + 1;
		format = "%.*s/%.*s";
	}
	(*end) = dest->len;
	...
	snprintf(dest->s, dest->len + 1, format, cfgt_sipmock_url.len, cfgt_sipmock_url.s,
			uuid.len, uuid.s);
	LM_DBG("SMOCK URL is %s\n", dest->s);
	return 0;
...
void cfgt_save_node(cfgt_node_p node)
{
	...
	jnode.s = srjson_Print(&node->jdoc, node->jdoc.root);
	if(jnode.s == NULL) {
	...
	if(fp) {
		pkg_free(dest.s);
		if(fputs(jnode.s, fp) < 0) {
			LM_ERR("failed writing to file: %s\n", strerror(errno));
		}
		fclose(fp);
		//node->jdoc.free_fn(jnode.s);
	...
	if (cfgt_sipmock_url.s) {
		dest.s = "";
		if(_cfgt_get_post_url(node->uuid, &dest, &dir) < 0) {
			LM_ERR("can't build HTTP URL\n");
			return;
		}
		dest.s[dir] = '\0';
		res = cfgt_post_report(jnode.s, dest.s);
		LM_INFO("*** node uuid:[%.*s] id:[%d] sent, CURLcode:[%d] ***\n",
			STR_FMT(&node->uuid), node->msgid, res);
		node->jdoc.free_fn(jnode.s);
		pkg_free(dest.s);
	} else {
		LM_INFO("*** SIPmock url is not set. done ***\n");
		node->jdoc.free_fn(jnode.s);
	}
...
int cfgt_post_report (char *obj, char *url)
{
	CURL *curl;
    CURLcode res;
	int lenb;
	...
	curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, lenb);
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, obj);
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "kamailio_cfgt/0.1");
